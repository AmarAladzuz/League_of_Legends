...
-- With temp table
drop table if exists #winning1

create table #winning1
(
name varchar(50),
win int,
total_wins numeric,
...
select *, (total_wins/total_picks)*100 as win_rate from #winning1
where win = 1
order by 5 desc
...
with lane_picks(position, name, pick_count, total_lane_picks)
as(
select position, name, count(name) as pick_count,  sum(count(name)) over(partition by position) as total_lane_picks from Lol..matches ma
join Lol..participants pa
on ma.id = pa.matchid
join Lol..champs ca
...
select *, (cast(pick_count as numeric)/cast(total_lane_picks as numeric))* 100 as lane_pick_rate from lane_picks
--where position = 'bot'
order by 5 desc